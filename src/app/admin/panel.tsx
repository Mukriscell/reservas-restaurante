...
import { Accessibility, FileSpreadsheet, LogOut, RefreshCw } from "lucide-react";
...
export default function PanelAdmin() {
  ...
  const totalPersonas = activas.reduce(
    (s, r) => s + r.adultos + r.ninos6a11 + r.ninos3a5,
    0
  );
  const totalCLP = activas.reduce((s, r) => s + r.totalEstimado, 0);
  const totalAbonado = activas.reduce((s, r) => s + r.abono, 0);
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-stone-600">
            {activas.length} reservas · {totalPersonas} personas · ingreso
